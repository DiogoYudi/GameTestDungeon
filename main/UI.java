...
public class UI {
    ...
    public void drawPlayerLife(){
        ...
        int i = 0;
        int iconSize = 32;
        int manaStartX = (gp.titleSize/2)-5;
        int manaStartY = 0;
        ...
        for(int i = 0; i < gp.monster[1].length; i++){
            ...
            if(monster != null && monster.inCamera()){
                if(monster.hpBarOn && !monster.boss){
                    double oneScale = (double)gp.titleSize/monster.maxLife;
                    double hpBarValue = oneScale*monster.life;

                    g2.setColor(new Color(35, 35, 35));         //Contorno da barra
                    g2.fillRect(monster.getScreenX()-1, monster.getScreenY()-16, gp.titleSize+2, 12);

                    g2.setColor(new Color(255, 0, 30));         //Barra de vida
                    g2.fillRect(monster.getScreenX(), monster.getScreenY()-15, (int) hpBarValue, 10);

                    monster.hpBarCounter++;
                    if(monster.hpBarCounter > 600){     //Apos 10 segundos (600 frames), a barra de vida desaparece
                        monster.hpBarCounter = 0;
                        monster.hpBarOn = false;
                    }
                }else if(monster.boss){
                    double oneScale = (double)gp.titleSize*8/monster.maxLife;
                    double hpBarValue = oneScale*monster.life;

                    int x = gp.screenWidth/2 - gp.titleSize*4;
                    int y = gp.titleSize*10;

                    g2.setColor(new Color(35, 35, 35));         //Contorno da barra
                    g2.fillRect(x-1, y-1, gp.titleSize*8+2, 22);
    ...
    public void drawDialogueScreen(){
        //Janela de dialogo
        int x = gp.titleSize*3;
        int y = gp.titleSize/2;
        int width = gp.screenWidth - (gp.titleSize*6);
    ...
    public void drawInventory(Entity entity, boolean cursor){
        ...
        if(cursor == true){
            ...
            int itemIndex = getItemIndexOnSlot(slotCol, slotRow);   //Vai pegar em que posição o item está (Por exemplo a espada está no slot 1 do inventario. Então está na posição 0 da arraylist)
    ...
    public void trade_buy(){
        ...
        if(itemIndex < npc.inventory.size()){
            ...
            String text = "" + price;
            x = getXForAlignToRightText(text, gp.titleSize*8-20);
            ...
            if(gp.keyH.enterPressed == true){
                if(npc.inventory.get(itemIndex).price > gp.player.coin){
                    ...
                    if(gp.player.canObtainItem(npc.inventory.get(itemIndex)) == true){
                        ...
                        if(npc.inventory.get(itemIndex).name == "Boots"){
                            gp.player.speed += 3;
                        }
                        gp.playSE(2);
                    }else{
    ...
    public void trade_sell(){
        ...
        if(itemIndex < gp.player.inventory.size()){
            ...
            String text = "" + price;
            x = getXForAlignToRightText(text, gp.titleSize*18-20);
            g2.drawString(text, x, y+34);
            ...
            if(gp.keyH.enterPressed == true){
                if(gp.player.inventory.get(itemIndex) == gp.player.currentWeapon || gp.player.inventory.get(itemIndex) == gp.player.currentShield){
                    ...
                    if(gp.player.inventory.get(itemIndex).name == "Boots"){
                        gp.player.defaultSpeed -= 3;
